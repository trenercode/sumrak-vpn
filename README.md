...
Рабочие ноды используют `nineseconds/mtg:2` с FakeTLS на публичном TCP-порту `443`. Установщик
генерирует secret формата `ee + 16 случайных байт + ya.ru`, поэтому соединение выглядит как
обычный TLS-трафик к домену маскировки. Нестандартные публичные порты и официальный
`telegrammessenger/proxy` в ограниченных сетях могут фильтроваться.

Proxy работает в host network без Docker NAT, чтобы тысячи долгоживущих Telegram-соединений
не переполняли conntrack. `mtg v2` не поддерживает sponsor tag; сохранённые поля sponsor tag
не применяются к конфигурации ноды.
