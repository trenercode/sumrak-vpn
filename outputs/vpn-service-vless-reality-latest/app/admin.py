...
from app.services import grant_subscription, load_user_with_devices, revoke_device
...
async def dashboard(request: Request, session: AsyncSession = Depends(get_session)):
    ...
    active_users = await session.scalar(
        ...
        .where(
            ...
            or_(User.trial_ends_at > now, User.subscription_ends_at > now),
        )
    )
...
async def users(request: Request, q: str = "", session: AsyncSession = Depends(get_session)):
    ...
    return templates(request).TemplateResponse(
        request, "users.html", {"users": result, "q": q}
    )
